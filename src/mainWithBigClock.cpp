    ...
    // 📺 Normal Mode
        ...
        case 7:
            ...
            if (localTime != LASTbigClockTimeStr)
            {
                ...
                tft.drawString(LASTbigClockTimeStr, 5, 65, 1);

                tft.setTextColor(TFT_GREEN);

                tft.drawString(localTime, 5, 65, 1);
            ...
            {
                colonVisible = !colonVisible; // toggle state

                // Choose color based on last NTP update result
                uint16_t colonColor = successFullTimeUpdate ? TFT_GREEN : TFT_RED;

                tft.setTextColor(colonVisible ? colonColor : TFT_BLACK);
                tft.drawString(":", 151, 65, 1);
